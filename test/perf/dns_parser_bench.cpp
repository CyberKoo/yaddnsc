...
std::vector<std::uint8_t> make_a_response(std::string_view qname) {
    ...
    buf.push_back(0x12);
    buf.push_back(0x34);  // ID
    buf.push_back(0x80);
    buf.push_back(0x80);   // flags
    ...
    buf.push_back(0xC0);
    buf.push_back(0x0C);   // name pointer to question
    write_u16_be(buf, 1);  // TYPE A
    write_u16_be(buf, 1);  // CLASS IN
    write_u16_be(buf, 0);
    write_u16_be(buf, 300);  // TTL
