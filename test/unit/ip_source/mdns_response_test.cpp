...
namespace {
    ...
    std::vector<std::uint8_t> response(std::initializer_list<std::tuple<std::string_view, std::uint16_t,
...
TEST(MdnsResponse, RejectsMalformedPacketAndInvalidRdata) {
    EXPECT_THROW(static_cast<void>(Mdns::parse_response(std::vector<std::uint8_t>{0, 1}, "printer.local", RecordKind::A)),
                 std::exception);
