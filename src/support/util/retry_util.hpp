...
namespace Utils::Retry {
    ...
    /// @param error_filter   Optional predicate: return true to retry, false to propagate.
    ...
    template<class R, class E, std::invocable<> Fn, typename Pred = std::nullopt_t>
        requires(std::same_as<Pred, std::nullopt_t> || detail::invocable_pred_for<Pred, E>) &&
                std::same_as<std::invoke_result_t<Fn>, std::expected<R, E> >
    [[nodiscard]] std::expected<R, E> retry_on_error(Fn &&func, unsigned retry, Pred &&error_filter = std::nullopt,
                                                     unsigned long backoff = 500, unsigned *actual_retries = nullptr) {
        unsigned counter = 0;
        while (true) {
            if (actual_retries)
                *actual_retries = counter;
