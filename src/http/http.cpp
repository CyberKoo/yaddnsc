...
[[nodiscard]] std::expected<Http::Response, Http::Error> read_response(
    ...
    for (;;) {
        ...
        auto result = Http::parse_response(std::string_view(buf.data(), total_read),
                                            "", MAX_BODY_SIZE);
        if (result) {
            const auto &headers = *result;

            // ── Phase 2: read body ──
            auto body = Http::read_body(stream, headers,
                                         std::span(buf.data(), total_read),
                                         MAX_BODY_SIZE, cancel_token);
            if (!body) {
        ...
        auto read_result = stream.read_some(
            std::span<std::uint8_t>(read_ptr, read_capacity), cancel_token);

        if (!read_result) {
            switch (read_result.error()) {
            case Transport::IoError::TIMEOUT:
                return std::unexpected(Http::Error::TIMEOUT);
            case Transport::IoError::CANCELLED:
                return std::unexpected(Http::Error::CANCELLED);
...
[[nodiscard]] Http::Error map_io_error(Transport::IoError e) noexcept {
    switch (e) {
    case Transport::IoError::TIMEOUT:           return Http::Error::TIMEOUT;
    case Transport::IoError::CANCELLED:         return Http::Error::CANCELLED;
    case Transport::IoError::CONNECTION_FAILED: return Http::Error::CONNECTION_FAILED;
    }
...
std::expected<Response, Error> exchange(
    Transport::Stream &stream,
