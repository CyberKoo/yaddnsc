...
public:
    ...
    [[nodiscard]] std::expected<std::vector<std::string>, DnsErrorInfo> run(
        std::span<const std::unique_ptr<ResolverBase>> batch);
...
std::expected<std::vector<std::string>, DnsErrorInfo> SingleResolverRunner::run(const std::string& host,
    ...
    auto result = Utils::Retry::retry_on_error<std::vector<std::string>, DnsErrorInfo>(
        ...
        max_retries, [](const DnsErrorInfo& e) { return is_retryable(e.code); }, backoff_ms, &actual_retries);
...
std::expected<std::vector<std::string>, DnsErrorInfo> ConcurrentRunner::run(const std::string& host,
                                                                            RecordKind type) const {
