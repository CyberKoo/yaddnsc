...
std::map<std::string, std::vector<InetAddress>> enumerate_interfaces() {
    ...
    for (auto* ifa = ifaddrs.get(); ifa != nullptr; ifa = ifa->ifa_next) {
        ...
        if (family == AF_INET) {
            ...
            auto bytes = std::span{reinterpret_cast<const std::uint8_t*>(&in->sin_addr.s_addr), Inet4Address::ADDR_LEN};
...
std::vector<Ipv4Subnet> get_ipv4_subnets(const std::string& iface_name) {
    ...
    for (auto* ifa = ifaddrs.get(); ifa != nullptr; ifa = ifa->ifa_next) {
        if (ifa->ifa_addr == nullptr || ifa->ifa_netmask == nullptr)
            continue;
        if (iface_name != ifa->ifa_name || ifa->ifa_addr->sa_family != AF_INET)
            continue;
        ...
        auto mask_bytes =
            std::span{reinterpret_cast<const std::uint8_t*>(&mask->sin_addr.s_addr), Inet4Address::ADDR_LEN};
