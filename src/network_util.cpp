...
std::vector<std::string> NetworkUtil::get_interfaces() {
    ...
    auto if_addrs = get_interface_addrs();
...
std::map<std::string, int> NetworkUtil::get_nif_ip_address(std::string_view nif) {
    auto all_nif_addrs = get_interface_addrs();
...
std::map<std::string, std::vector<NetworkUtil::interface_addrs_t>> NetworkUtil::get_interface_addrs() {
    std::map<std::string, std::vector<interface_addrs_t>> addresses;
    ...
    for (auto ifa = ifaddrs.get(); ifa != nullptr; ifa = ifa->ifa_next) {
        if (ifa->ifa_addr != nullptr) {
            ...
            if (addresses.find(ifa->ifa_name) == addresses.end()) {
                addresses.insert({ifa->ifa_name, {}});
            }
            ...
            addresses[ifa->ifa_name].emplace_back(interface_addrs_t{host, ifa->ifa_addr->sa_family});
