...
struct ScopedMembership
{
  ...
  explicit ScopedMembership(Socket& sock,
      ...
      : sock_(sock)
  {
    if constexpr (std::is_same_v<Tag, Ipv6Tag>) {
      ...
      if (auto err = sock_.try_set_option(IPPROTO_IPV6, IPV6_JOIN_GROUP, mreq_)) {
        throw std::runtime_error(fmt::format(R"(mDNS IPV6_JOIN_GROUP failed for "{}": {})", hostname, errno_str(err)));
      }
    } else {
  ...
  ~ScopedMembership()
  {
    ...
    if (auto err = sock_.try_set_option(level, leave_opt, mreq_)) {
      SPDLOG_WARN(R"(mDNS {} failed: {})", std::is_same_v<Tag, Ipv6Tag> ? "IPV6_LEAVE_GROUP" : "IP_DROP_MEMBERSHIP",
                  errno_str(err));
    }
  }
...
template<IpVersionTag Tag>
[[nodiscard]] unsigned int setup_multicast_options(Socket& sock,
                                                   const std::string& hostname,
                                                   const std::string& interface)
{
...
[[nodiscard]] std::vector<InetAddress> recv_and_parse(Socket& sock, RecordKind type, const std::string& hostname)
{
  ...
  if (ready <= 0) {
    throw std::runtime_error(fmt::format(R"(mDNS no response for "{}" within {}ms)", hostname, MDNS_TIMEOUT_MS));
  }
                                                    ...
                                                    const std::string& interface)
{
  ...
  SPDLOG_DEBUG(R"(mDNS resolving "{}" (type {}) on interface "{}")", hostname,
               std::is_same_v<Tag, Ipv6Tag> ? "AAAA" : "A", iface_label);
  ...
  if (!interface.empty()) {
    ...
    if (auto err = sock.try_set_option_raw(SOL_SOCKET, SO_BINDTODEVICE, interface.c_str(),
      ...
      SPDLOG_WARN(R"(mDNS setsockopt(SOL_SOCKET, SO_BINDTODEVICE) failed for "{}": {})", hostname, errno_str(err));
    }
    ...
    SPDLOG_DEBUG(R"(mDNS interface "{}" will be used via multicast options for "{}")", interface, hostname);
#else
