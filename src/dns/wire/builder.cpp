...
public:
  ...
  void encode_name(std::string_view name)
  {
    ...
    if (encoded_size > 255) {
      throw DnsPacketException(fmt::format("Domain name \"{}\" is too long ({} octets, max 255)", name, encoded_size));
    }
...
build_flags(bool qr, std::uint8_t opcode, bool aa, bool tc, bool rd, bool ra, std::uint8_t rcode) noexcept
{
  return static_cast<std::uint16_t>((qr ? FLAG_QR : 0) | (static_cast<std::uint16_t>(opcode & 0x0F) << 11) |
                                    (aa ? FLAG_AA : 0) | (tc ? FLAG_TC : 0) | (rd ? FLAG_RD : 0) | (ra ? FLAG_RA : 0) |
                                    (rcode & 0x0F));
}
...
QueryBuilder::QueryBuilder() noexcept
    : id_([]() {
      ...
      qr_(false), opcode_(0), aa_(false), tc_(false), rd_(true), ra_(false), rcode_(0)
{
}
...
QueryBuilder& QueryBuilder::add_question_raw_qclass(std::string_view qname, RecordType qtype, std::uint16_t raw_qclass)
{
  questions_.push_back(PendingQuestion{
      .qname = std::string(qname),
      .qtype = qtype,
                                     ...
                                     std::span<const EdnsOption> options)
{
  edns_ = EdnsConfig{
      ...
      .version = version,
      .dnssec_ok = dnssec_ok,
      .options = std::vector<EdnsOption>(options.begin(), options.end()),
  };
  return *this;
}
...
std::vector<std::uint8_t> QueryBuilder::build() const
{
  ...
  if (edns_.has_value()) {
    ...
    if (edns.version != 0) {
      throw DnsPacketException(fmt::format("EDNS version {} is not supported (only version 0 is valid)",
    ...
    const auto ttl = static_cast<std::uint32_t>((static_cast<std::uint32_t>(edns.version) << 16) |
