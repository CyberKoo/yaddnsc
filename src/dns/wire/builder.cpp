...
[[nodiscard]] std::uint16_t build_flags(bool qr, std::uint8_t opcode, bool aa,
                                         bool tc, bool rd, bool ra,
                                         std::uint8_t rcode) noexcept {
    return static_cast<std::uint16_t>(
           (qr ? FLAG_QR : 0)
         | (static_cast<std::uint16_t>(opcode & 0x0F) << 11)
         | (aa ? FLAG_AA : 0)
         | (tc ? FLAG_TC : 0)
         | (rd ? FLAG_RD : 0)
         | (ra ? FLAG_RA : 0)
...
QueryBuilder& QueryBuilder::add_question(std::string_view qname,
                                          RecordType qtype,
                                          RecordClass qclass) {
...
std::vector<std::uint8_t> QueryBuilder::build() const {
    ...
    if (edns_.has_value()) {
        ...
        if (edns.udp_payload_size < 512) {
            throw DnsPacketException(
                fmt::format("EDNS UDP payload size {} is too small (minimum 512)",
