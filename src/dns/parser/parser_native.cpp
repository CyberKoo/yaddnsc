...
std::string DNS::RecordParser::decompress_name(const std::span<const std::uint8_t> wire,
                                             size_t &offset) {
...
DNS::RecordParser::parse_edns(const ResourceRecord &rr) {
...
std::string
DNS::RecordParser::rdata_to_string(const ResourceRecord &rr, const std::span<const std::uint8_t> wire) {
    ...
    switch (rr.type) {
        case static_cast<std::uint16_t>(RecordType::A):
            if (rdlen != 4) {
                throw DnsLookupException(
                    fmt::format("Invalid A record RDATA length: {}", rdlen), DnsError::PARSE);
            }
        ...
        case static_cast<std::uint16_t>(RecordType::AAAA):
            if (rdlen != 16) {
                throw DnsLookupException(
                    fmt::format("Invalid AAAA record RDATA length: {}", rdlen), DnsError::PARSE);
            }
...
DNS::ParsedMessage
DNS::RecordParser::parse_message(const std::span<const std::uint8_t> data) {
    ...
    for (uint16_t i = 0; i < m.qdcount; ++i) {
        ...
        if (offset + QUESTION_FIXED_SIZE > data.size()) {
            throw DnsLookupException(
                fmt::format("DNS question section truncated at offset {}", offset), DnsError::PARSE);
        }
    ...
    auto parse_rr = [&]() -> ResourceRecord {
        ...
        if (offset + RR_FIXED_SIZE > data.size()) {
            throw DnsLookupException(
                fmt::format("DNS RR header truncated at offset {}", offset), DnsError::PARSE);
        }
...
DNS::ParsedResponse
DNS::RecordParser::parse_response(const std::span<const std::uint8_t> data,
...
DNS::FormattedResponse
DNS::RecordParser::parse_strings(const std::span<const std::uint8_t> data,
                                 [[maybe_unused]] const std::string &host) {
    RecordParser parser(data);
