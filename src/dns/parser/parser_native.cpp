  ...
  while (true) {
    if (current >= wire_len) {
      throw DnsLookupException(
          fmt::format("DNS name decompression: offset {} beyond wire length {}", current, wire_len), DnsError::PARSE);
    }
    ...
    if ((label_len & 0xC0) == 0xC0) {
      ...
      const auto ptr_offset = static_cast<size_t>(((static_cast<size_t>(label_len) & 0x3F) << 8) | wire[current + 1]);

      if (!visited.insert(ptr_offset).second) {
        throw DnsLookupException(
            fmt::format("DNS name decompression: repeated pointer to offset {} (cycle)", ptr_offset), DnsError::PARSE);
      }

      if (indirections++ >= MAX_POINTER_DEPTH) {
        throw DnsLookupException(fmt::format("DNS name decompression: too many indirections ({})", MAX_POINTER_DEPTH),
                                 DnsError::PARSE);
      }
    ...
    if (current + label_len > wire_len) {
      throw DnsLookupException(
          fmt::format("DNS name decompression: label of length {} extends past wire end", label_len), DnsError::PARSE);
    }
...
{
  const auto rdata = std::span<const std::uint8_t>{rr.rdata};
  const auto rdlen = rdata.size();
...
DNS::ParsedMessage DNS::RecordParser::parse_message(const std::span<const std::uint8_t> data)
{
  if (data.size() < HEADER_SIZE) {
    throw DnsLookupException(fmt::format("DNS packet too short: {} bytes (minimum {} bytes)", data.size(), HEADER_SIZE),
                             DnsError::PARSE);
  }
  ...
  for (uint16_t i = 0; i < m.qdcount; ++i) {
    ...
    if (offset + QUESTION_FIXED_SIZE > data.size()) {
      throw DnsLookupException(fmt::format("DNS question section truncated at offset {}", offset), DnsError::PARSE);
    }
  ...
  for (uint16_t i = 0; i < m.arcount; ++i) {
    ...
    if (rr.type == static_cast<std::uint16_t>(RecordType::OPT) && rr.name.size() == OPT_NAME_SIZE && rr.qclass > 0) {
...
DNS::RecordParser::RecordParser(const std::span<const std::uint8_t> data) : wire_(data)
{
  ...
  SPDLOG_TRACE(R"(DNS message parser initialised (message size: {}, answer count: {}))", data.size(), message_.ancount);
}
...
std::string DNS::RecordParser::parse_record(size_t index) const
{
  ...
  SPDLOG_TRACE(R"(Parsing DNS record #{}: type={} ({}), name="{}")", index, type_name.empty() ? "?" : type_name.data(),
