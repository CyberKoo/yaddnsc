...
std::string DNS::RecordParser::parse_record(size_t index) const {
    ...
    const auto msg_span = std::span<const std::uint8_t>{ns_msg_base(message_),
                                                         static_cast<size_t>(ns_msg_end(message_) - ns_msg_base(message_))};
...
std::string DNS::RecordParser::parse_txt_record(const std::span<const std::uint8_t> rdata) {
    ...
    if (rdata.size() < static_cast<std::size_t>(1) + length) {
        throw DnsLookupException(
            fmt::format("Invalid TXT record: declared length {} exceeds remaining data length {}", length, rdata.size() - 1)
        );
    }
