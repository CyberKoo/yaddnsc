...
std::string DNS::RecordParser::parse_record(size_t index) const
{
  ...
  if (ns_parserr(&message_, ns_s_an, static_cast<int>(index), &dns_resource)) {
    throw DnsLookupException(fmt::format("An error occurred when parsing DNS resource at index {}, detail: {}", index,
  ...
  if (response.rcode == Rcode::NOERROR) {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      if (ns_parserr(&parser.message_, ns_s_an, static_cast<int>(i), &dns_resource)) {
        throw DnsLookupException(fmt::format("An error occurred when parsing DNS resource at index {}", i),
                                 DnsError::PARSE);
      }
