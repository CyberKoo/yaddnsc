...
[[nodiscard]] AddrResult make_addr(const Config::DnsServer& server)
{
  ...
  if (!parsed) {
    throw DnsLookupException(fmt::format(R"(Invalid DNS server address "{}" — must be an IP address)", server.address),
                             DnsError::CONFIG);
  }
...
[[noreturn]] void rethrow_socket(const SocketException& e, const char* context, std::uint64_t resolver_id)
{
  ...
  if (e.get_errno() == ETIMEDOUT || e.get_errno() == EAGAIN) {
    throw DnsLookupException(fmt::format(R"(Resolver #{} DNS lookup timed out)", resolver_id), DnsError::RETRY);
  }
  if (e.get_errno() == ECANCELED) {
    throw DnsLookupException(fmt::format(R"(Resolver #{} DNS lookup cancelled)", resolver_id), DnsError::CANCELLED);
  }
  throw DnsLookupException(fmt::format(R"(Resolver #{} DNS lookup: {} {})", resolver_id, context, e.what()),
                           DnsError::CONNECTION);
}

// ── UDP query ──
[[nodiscard]] std::vector<std::uint8_t> query_udp(const AddrResult& addr,
                                                  const std::vector<std::uint8_t>& query_packet,
                                                  int cancel_fd,
...
[[nodiscard]] std::vector<std::uint8_t> query_tcp(const AddrResult& addr,
                                                  const std::vector<std::uint8_t>& query_packet,
                                                  int cancel_fd,
                                                  std::uint64_t resolver_id)
{
  try {
    ...
    if (rsp_len == 0 || rsp_len > MAX_DNS_PACKET_SIZE) {
      throw DnsLookupException(fmt::format("Invalid DNS response length: {}", rsp_len), DnsError::PARSE);
    }
...
struct ClassicResolver::Impl
{
  ...
  [[nodiscard]] std::expected<std::vector<std::uint8_t>, DnsLookupException> query(const std::string& host_str,
                                                                                   RecordKind type,
                                                                                   int cancel_fd = -1) const;
...
std::expected<std::vector<std::uint8_t>, DnsLookupException> ClassicResolver::Impl::query(const std::string& host_str,
                                                                                          RecordKind type,
                                                                                          int cancel_fd) const
{
  try {
...
ClassicResolver::ClassicResolver(Config::DnsServer server) : impl_(std::make_unique<Impl>(std::move(server), get_id()))
{
}

ClassicResolver::~ClassicResolver() = default;

std::expected<std::vector<std::uint8_t>, DnsLookupException> ClassicResolver::query(const std::string& host,
                                                                                    RecordKind type,
...
[[maybe_unused]] DnsResolverRegistry::Registrar _classic("",
                                                         [](const Config::DnsServer& server)
                                                             -> std::shared_ptr<ResolverBase> {
