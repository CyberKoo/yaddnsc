...
struct DotResolver::Impl
{
  ...
  // ── Member functions ──
  [[nodiscard]] std::expected<std::vector<std::uint8_t>, DnsLookupException> query(const std::string& host,
                                                                                   RecordKind type,
...
std::expected<std::vector<std::uint8_t>, DnsLookupException> DotResolver::Impl::query(const std::string& host,
                                                                                      RecordKind type,
                                                                                      int cancel_fd) const
{
  try {
    ...
    if (!bio_send_all(bio, std::span{wire}, cancel_fd)) {
      ...
      if (!bio_send_all(bio, std::span{wire}, cancel_fd)) {
        persistent_bio_.reset();
        throw DnsLookupException(fmt::format(R"(DoT failed to send query to "{}" after reconnect)", target),
                                 DnsError::CONNECTION);
      }
    }
    ...
    if (!bio_read_exact(bio, resp_len_buf, cancel_fd)) {
      persistent_bio_.reset();
      throw DnsLookupException(fmt::format(R"(DoT failed to read response length from "{}")", target),
                               DnsError::CONNECTION);
    }
    ...
    if (resp_len == 0) {
      persistent_bio_.reset();
      throw DnsLookupException(fmt::format(R"(DoT server "{}" returned zero-length response)", target),
                               DnsError::PARSE);
    }
    ...
    if (!bio_read_exact(bio, std::span{response}, cancel_fd)) {
      persistent_bio_.reset();
      throw DnsLookupException(fmt::format(R"(DoT failed to read response body from "{}")", target),
                               DnsError::CONNECTION);
    }
    ...
    SPDLOG_DEBUG(R"(Resolver #{} DoT query to "{}" succeeded ({} bytes) for "{}")", id_, target, response.size(), host);
...
auto DotResolver::Impl::create_ssl_ctx() -> SslCtxPtr
{
  ...
  if (SSL_CTX_set_default_verify_paths(ctx.get()) != 1) {
    ...
    if (!Utils::Cert::get_system_ca_path().and_then([raw = ctx.get()](const auto& path) -> std::optional<std::string> {
...
auto DotResolver::Impl::connect(SSL_CTX* ctx, const std::string& server, std::uint16_t port) -> BioPtr
{
  ...
  if (!is_ip && !Utils::is_valid_domain(server)) {
    throw DnsLookupException(fmt::format(R"(Invalid DoT server: "{}" (not a valid IP or domain name))", server),
                             DnsError::CONFIG);
  }
  ...
  for (;;) {
    ...
    if (now >= deadline) {
      throw DnsLookupException(fmt::format(R"(DoT connection timeout ({}s) for "{}")", CONNECT_TIMEOUT.count(), target),
                               DnsError::CONNECTION);
    }

    const auto fd = BIO_get_fd(bio.get(), nullptr);
    if (fd == -1) {
      throw DnsLookupException(fmt::format(R"(DoT failed to get socket fd for "{}")", target), DnsError::CONNECTION);
    }
...
std::expected<std::vector<std::uint8_t>, DnsLookupException> DotResolver::query(const std::string& host,
                                                                                RecordKind type,
