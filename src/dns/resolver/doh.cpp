...
DohResolver::Impl::query(const std::string& host, RecordKind type, [[maybe_unused]] int cancel_fd) const
{
  try {
    ...
    HttpRequest req{.content_type = DOH_CONTENT_TYPE,
    ...
    if (!response) {
      throw DnsLookupException(fmt::format(R"(DoH query to "{}" failed: {})", server_, response.error()),
                               DnsError::CONNECTION);
    }
    ...
    if (response->status_code != 200) {
      ...
      throw DnsLookupException(
          fmt::format(R"(DoH server "{}" returned HTTP status {})", server_, response->status_code), ec);
    }
    ...
    // Validate Content-Type per RFC 8484 §6.
    {
      ...
      if (!valid_ct) {
        throw DnsLookupException(fmt::format(R"(DoH server "{}" returned unexpected Content-Type)", server_),
                                 DnsError::PARSE);
      }
    }
...
std::expected<std::vector<std::uint8_t>, DnsLookupException> DohResolver::query(const std::string& host,
                                                                                RecordKind type,
...
[[maybe_unused]] DnsResolverRegistry::Registrar _doh(
    ...
    [](const Config::DnsServer& server) -> std::shared_ptr<ResolverBase> {
      ...
      auto opts = HttpClientOptions{.connection_timeout = std::chrono::seconds(1),
