...
[[nodiscard]] std::expected<R, E> retry_on_error(Fn&& func,
                                                 unsigned retry,
                                                 Pred&& e_filter = std::nullopt,
                                                 unsigned long backoff = 500,
                                                 unsigned* actual_retries = nullptr)
{
  static_assert(std::is_same_v<decltype(func()), std::expected<R, E>>, "Fn must return std::expected<R, E>");
