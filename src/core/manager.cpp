...
public:
    explicit Impl(Config::config config) : config_(std::move(config)) {
        ...
        if (dns_servers.size() > 1) {
            SPDLOG_INFO("Configured {} custom resolver(s) in {} mode", dns_servers.size(),
                        config_.resolver.strategy == Config::resolver_strategy::FALLBACK ? "fallback" : "concurrent"
            );
        }
    ...
    void run() {
        ...
        // Discard the remaining schedule — the next cycle's task was already
        // re-queued and is still sleeping; just cancel it.
        schedule_ = {};
