...
namespace {
    ...
    private:
        [[nodiscard]] static HandlePtr open_handle(const std::string &path);

        static void verify_magic(const HandlePtr &handle, std::string_view driver_name);
    ...
    void DriverModule::verify_compiler(const HandlePtr &handle, std::string_view driver_name) {
        auto drv_hash_func = resolve_symbol<std::uint64_t()>(handle, "yaddnsc_drv_compiler_id_hash");
    ...
    if (!got.is_compatible_with(required)) {
        throw BadDriverException(
            fmt::format("Driver {} ABI {}.{}.{} incompatible with host {}.{}.{}: "
                        ...
                        driver_lib_name, got.major, got.minor, got.patch, required.major, required.minor,
                        required.patch,
