...
[[nodiscard]] std::filesystem::path resolve_driver_path(const std::filesystem::path& base_dir,
...
void load_auto_discover(DriverManager& driver_manager, const Config::AppConfig& config)
{
  if (!config.driver.load.empty()) {
    SPDLOG_WARN("auto_discover is enabled, ignoring manual load list with {} entry(ies)", config.driver.load.size());
  }
