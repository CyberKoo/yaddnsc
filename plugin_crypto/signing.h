...
[[nodiscard]] std::vector<std::uint8_t> sha256(std::span<const std::uint8_t> data) noexcept;

/// Compute the SHA-1 digest of @p data.
[[nodiscard]] std::vector<std::uint8_t> sha1(std::span<const std::uint8_t> data) noexcept;
...
[[nodiscard]] std::string hex_encode(std::span<const std::uint8_t> data) noexcept;

/// Encode @p data as a Base64 string (standard alphabet, no padding).
[[nodiscard]] std::string base64_encode(std::span<const std::uint8_t> data) noexcept;

// ── Time helpers ─────────────────────────────────────────────────────────

/// Current UTC time formatted as "YYYYMMDDTHHmmSSZ".
[[nodiscard]] std::string iso8601_timestamp() noexcept;
