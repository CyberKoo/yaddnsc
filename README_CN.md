...
| 字段        | 类型      | 说明                                                                                                                                |
|-----------|---------|-----------------------------------------------------------------------------------------------------------------------------------|
| `address` | string  | **传统 DNS**：纯 IP 地址（如 `1.1.1.1`）。**DNS-over-HTTPS (DoH)**：完整的 HTTPS URL，**包含路径**，例如 `https://1.1.1.1/dns-query`。必须以 `https://` 开头。**DNS-over-TLS (DoT)**：`tls://` URI（如 `tls://1.1.1.1`）。 |
