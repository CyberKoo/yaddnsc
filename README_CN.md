...
- **自定义 DNS 解析器** — 可选使用特定 DNS 服务器代替系统默认解析器。支持传统 DNS、DNS-over-HTTPS (DoH) 和 DNS-over-TLS (DoT)，并提供可配置的查询策略。详见 [DNS 解析器](#dns-解析器)。
...
**线程模型：** 单调度器线程（内联在 `Scheduler::run()` 中，由 `Manager::run()` 调用）维护一个 `SubdomainEntry` 最小堆（按截止时间排序）。子域名到期时，调度器将其弹出堆，将 `Updater::process(task)` 提交到共享线程池，然后以新的截止时间重新入队。调度器在条件变量上等待，直到最近截止时间或收到停止请求。关闭时丢弃堆中剩余任务，在 `Scheduler` 析构函数中等候线程池中所有任务完成后再退出。

**信号处理：** 专用 `std::jthread` 在 `sigwait()` 上等待 `SIGINT`/`SIGTERM`。收到信号后调用 `stop_source.request_stop()`，触发 `std::stop_callback` 通知调度器的条件变量，唤醒循环使其跳出。待处理的线程池任务将在 `Scheduler` 析构时排空。

**Updater::process()**（在线程池中执行）对每个子域名依次执行：

1. **解析驱动** — 从 `DriverManager` 按名称查找驱动插件。
2. **获取本地 IP** — 通过 IP 来源抽象层（`IpSourceBase`）解析，从本地网卡读取（`InterfaceIpSource`）、从外部 URL 获取（`HttpIpSource`），或通过 mDNS 发现局域网设备（`MdnsIpSource`）。由 `IpSourceFactory` 根据子域名配置自动创建对应的实现。
...
**HTTP 抽象层：** 所有服务商 API 通信均通过 `HttpClient` 接口进行。两个具体实现：`TransientHttpClient` 每次请求创建新的 [cpp-httplib](https://github.com/yhirose/cpp-httplib) 客户端（更新器执行驱动时使用），`PersistentHttpClient` 在多次调用间复用单一连接（DoH 解析器内部使用）。地址族和网络接口通过 `HttpClientOptions` 按需配置。
...
**Legacy 设备** — 如果工具链版本过低（GCC < 14 或 Clang < 18），请使用 `v0.x`（legacy）分支（C++17、CMake 3.14+、OpenSSL 1.1.x）。该分支处于维护模式，不会添加新功能，但会修复关键 bug。该分支也支持 DoT/DoH 解析器。

**Alpine Linux** — musl 提供的是降级版的经典 `resolv` stub，缺少现代特性，且与 glibc 的解析器相比存在性能瓶颈和功能限制。强烈建议在 Alpine 上将解析器配置为 **DoT**（DNS over TLS）或 **DoH**（DNS over HTTPS），而非系统解析器。
...
| `address`           | string      | DNS 服务器地址（旧字段，仅当 `servers` 为空时生效）。支持的地址格式详见 [DNS 解析器](#dns-解析器)。 |
| `port`              | integer     | DNS 服务器端口，通常为 53（旧字段，仅当 `servers` 为空时生效）。DoH/DoT 时忽略。                |
| `servers`           | DnsServer[] | DNS 服务器列表。支持的地址格式及各类型的端口行为详见 [DNS 解析器](#dns-解析器)。                     |
...
| 字段        | 类型      | 说明                                                                                                                                |
|-----------|---------|-----------------------------------------------------------------------------------------------------------------------------------|
| `address` / `ipaddress` | string  | DNS 服务器地址。格式取决于解析器类型——详见 [DNS 解析器](#dns-解析器)。也可写作 `ipaddress` 以兼容旧配置。 |
| `port`    | integer | DNS 服务器端口，通常为 53。DoH/DoT 时忽略——详见 [DNS 解析器](#dns-解析器)。                        |

当 `servers` 数组存在且非空时，旧字段 `address` 和 `port` 被忽略。在不支持 `res_nquery()` 的平台（如某些 musl 构建）上，自定义服务器不可用，始终使用系统解析器。
...
| `ip_source`        | string  | IP 来源策略：`"interface"`、`"http"`（别名 `"url"`）或 `"mdns"`。详见 [IP 来源说明](#ip-来源说明)。 |
| `ip_source_param`  | string  | 来源相关参数（`"http"` 为 URL，`"mdns"` 为 mDNS 主机名）。详见 [IP 来源说明](#ip-来源说明)。`"interface"` 来源时忽略。 |
...
> **注意：** `interface` 字段**只对** `"interface"` 来源为必填项。对于 `"http"` 和 `"mdns"` 来源为**可选项**——指定时 HTTP 请求或 mDNS 查询绑定到该网卡；省略时使用系统默认路由。
...
| 字段               | 必需 | 说明                                          |
|--------------------|------|----------------------------------------------|
| `interface`        | 是   | 要读取 IP 的网卡名称（如 `eth0`、`wlan0`）。         |
| `ip_source_param`  | 否   | 此来源忽略该字段。                                |
...
| 字段               | 必需 | 说明                                              |
|--------------------|------|--------------------------------------------------|
| `interface`        | 否   | 绑定 HTTP 请求的网卡。省略时使用系统默认路由。          |
| `ip_source_param`  | 是   | 返回 IP 地址的 HTTP(S) URL，响应体需包含 IP 地址文本。 |
...
| 字段               | 必需 | 说明                                              |
|--------------------|------|--------------------------------------------------|
| `interface`        | 否   | 发送 mDNS 查询的网卡。省略时使用默认路由。              |
| `ip_source_param`  | 是   | 要查询的 mDNS 主机名。必须符合 RFC 6762 §3 规范，以 `.local`（或 `.local.`）结尾。 |

> **平台说明：** 客户端通过 UDP 多播直接发送原始 mDNS 查询报文，发送端无需任何 mDNS 守护进程。**目标设备**必须运行 mDNS 响应端才能回复查询（Linux 上需运行 Avahi 或启用 mDNS 的 systemd-resolved；macOS 自带 mDNSResponder；大多数嵌入式设备内置支持）。已在 Linux、macOS 和 FreeBSD 上完成测试。
...
通过 UDP（大响应时使用 TCP）在指定 IP 和端口上使用标准 DNS 协议。
...
| 字段      | 说明                                   |
|---------|--------------------------------------|
| `address` | 纯 IP 地址（如 `"1.1.1.1"`）。IPv6 地址**不要**带方括号。 |
| `port`    | UDP/TCP 端口，通常为 53。                     |

> **IPv6 说明：** IPv6 地址不要带方括号，例如 `"2606:4700:4700::1111"`。方括号用于 URI 字面量（如 `[::1]:53`），但 `inet_pton()` 解析时需要纯地址格式。

#### DNS-over-HTTPS (DoH)

通过 HTTPS POST 加密 DNS 查询（RFC 8484）。
...
| 字段      | 说明                                             |
|---------|------------------------------------------------|
| `address` | 完整的 HTTPS URL，**包含路径**（如 `"https://1.1.1.1/dns-query"`）。必须以 `https://` 开头。 |
| `port`    | 忽略——HTTPS 默认使用 443 端口。                        |

> 地址必须是包含完整路径的 URL。代码**不会**自动追加 `/dns-query`，请确保配置完整。

#### DNS-over-TLS (DoT)

通过 TLS 加密 DNS 查询（RFC 7858）。
...
| 字段      | 说明                         |
|---------|----------------------------|
| `address` | `tls://` URI（如 `"tls://1.1.1.1"`）。 |
| `port`    | 忽略——TLS 默认使用 853 端口。          |
...
自定义 DNS 服务器依赖 `res_nquery()` 函数。在不支持此函数的平台（如某些基于 musl 的构建）上，始终使用系统解析器，自定义服务器配置将被忽略。
...
`HttpClient` 由更新器在每次更新时构造为默认的 `TransientHttpClient`。覆盖 `execute()` 的驱动直接使用此客户端。如需要地址族或网络接口绑定，可在覆盖方法中构造带自定义 `HttpClientOptions` 的 `TransientHttpClient`（或 `PersistentHttpClient`）。
...
`HttpResult` 是 `std::expected<HttpResponse, std::string>` 的类型别名。通过隐式布尔转换判断成功，或用 `error()` 获取错误字符串：
