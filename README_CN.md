...
- **自定义 DNS 解析器** — 可选使用特定 DNS 服务器代替系统默认解析器。支持**传统 DNS**（纯 IP + 端口）和 **DNS-over-HTTPS (DoH)**（完整 HTTPS URL，如 `https://1.1.1.1/dns-query`）。支持可配置的查询策略——**fallback**（依次尝试解析器，失败时切换到下一个）或 **concurrent**（同时向所有配置的解析器发起查询，取最快成功响应）。
