...
| `interface`        | string  | 网卡接口名称（如 `eth0`）。`"interface"` 和 `"http"` 两种 IP 来源均必填。 |
| `ip_type`          | string  | **已废弃——被忽略。** 地址族现在由 `type` 自动推导（A → IPv4，AAAA → IPv6）。 |
| `ip_source`        | string  | IP 来源：`"interface"`（从本地网卡读取）或 `"http"`（通过 HTTP 获取；`"url"` 也接受用于向后兼容）            |
| `ip_source_param`  | string  | 当 `ip_source` 为 `"http"` 时：HTTP(S) URL。`"interface"` 来源时忽略                        |
...
> **注意：** `interface` 字段对两种 IP 来源均为**必填项**。`ip_source` 为 `"interface"` 时直接从网卡读取 IP；为 `"http"` 时 HTTP 请求绑定到该网卡以获取 IP。
