...
单元测试目前受到核心组件紧耦合的限制——IP 来源、DNS 解析器、驱动和调度器之间互相直接依赖，没有可注入的接口。计划通过核心重构解耦这些依赖，重构完成后即引入单元测试。
...
项目提供了 systemd 服务文件 `yaddnsc.service`，集成了启动前配置验证（`config test`）、安全加固（DynamicUser、ProtectSystem、ProtectHome），并支持通过 `/etc/default/yaddnsc` 覆盖配置路径等环境变量：
