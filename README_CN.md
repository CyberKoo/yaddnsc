...
- **自定义 DNS 解析器** — 可选使用特定 DNS 服务器代替系统默认解析器。支持**传统 DNS**（纯 IP + 端口）和 **DNS-over-HTTPS (DoH)**（完整 HTTPS URL，如 `https://1.1.1.1/dns-query`）。支持**多服务器并发查询** + 自动容灾（同时向所有配置的解析器发起查询，取最快响应）。
...
2. 获取本地 IP（接口或 URL）
3. DNS 查询（DnsResolver / DohResolver）
4. 对比 IP，无变化则跳过
5. 创建 HttplibHttpClient
6. 调用 driver.execute()"]
...
2. **获取本地 IP** — 从网卡接口读取（`NetworkManager`）或从外部 URL 获取。
3. **DNS 查询** — 使用 `DnsResolver`（UDP/TCP）或 `DohResolver`（HTTPS POST, RFC 8484）查询当前 DNS 记录。
4. **对比** — IP 无变化则跳过更新（除非设置了 `force_update`）。
5. **创建 `HttplibHttpClient`** — 为每个任务创建绑定了接口和地址族的 HTTP 客户端。
6. **调用 `driver.execute()`** — 委托给驱动插件，执行 `generate_request()` → `HttpClient::send()` → `check_response()`。

**HTTP 抽象层：** 所有服务商 API 通信均通过 `HttpClient` 接口进行。具体实现 `HttplibHttpClient` 在内部封装了 [cpp-httplib](https://github.com/yhirose/cpp-httplib)。它在 `Updater::process()` 内部按任务创建，绑定子域名配置的接口和地址族。
...
| `address`           | string      | DNS 服务器地址（旧字段，仅当 `servers` 为空时生效）。传统 DNS 使用纯 IP 地址。**DoH** 请使用完整的 HTTPS URL，含路径（如 `https://1.1.1.1/dns-query`）。 |
| `port`              | integer     | DNS 服务器端口，通常为 53（旧字段，仅当 `servers` 为空时生效）。当 `address` 为完整 HTTPS URL 时忽略。                                         |
| `servers`           | DnsServer[] | DNS 服务器列表，用于冗余容灾。每项包含 `address`（字符串）和 `port`（整数，默认 53）字段。配置多个服务器时，会**并发**发起所有查询，取最快成功响应。如果所有服务器均失败，则传播最终错误。     |

`servers` 数组中每个 `DnsServer` 项的结构如下：

| 字段        | 类型      | 说明                                                                                                                                |
|-----------|---------|-----------------------------------------------------------------------------------------------------------------------------------|
| `address` | string  | **传统 DNS**：纯 IP 地址（如 `1.1.1.1`）。**DNS-over-HTTPS (DoH)**：完整的 HTTPS URL，**包含路径**，例如 `https://1.1.1.1/dns-query`。必须以 `https://` 开头。 |
| `port`    | integer | DNS 服务器端口，通常为 53。**DoH 忽略此字段**（HTTPS 默认使用 443 端口）。                                                                                |

> **DoH 说明：** 当 `address` 以 `https://` 开头时，视为 DNS-over-HTTPS 解析器。地址必须是完整的 URL，包含完整路径——`/dns-query` 是 RFC 8484 定义的标准 DoH 端点，也支持任意自定义路径。代码**不会**自动追加 `/dns-query`，请确保配置完整。
...
| `interface`        | string  | 网卡接口名称（如 `eth0`）。`ip_source` 为 `"interface"` 时必须设置；`ip_source` 为 `"url"` 时可省略 |
...
> **注意：** `interface` 字段为可选项。当 `ip_source` 为 `"url"` 时，可以完全省略 `interface`，HTTP 请求将不绑定特定网卡。当 `ip_source` 为 `"interface"` 时，`interface` 为必填项。
...
`HttpClient` 由初始核⼼以正确的地址族（IPv4/IPv6）和⽹络接⼝初始化，这些值来⾃每个⼦域名的配置。覆盖 `execute()` 的驱动可以在多次调⽤间通过 `set_address_family()` 切换地址族。
