...
| `YADDNSC_USE_NATIVE_DNS`      | OFF                                           | [实验性] 使用内置 DNS 查询和解析器（不依赖 libresolv）。稳定后将改为默认。长期目标：彻底摆脱 libresolv。 |
...
> **实验性说明：** 内置 resolver/parser（`ON`）仍在打磨中。稳定后将改为默认值。长期目标是彻底移除对 libresolv 的依赖。

两种模式下，DNS 报文解析均为自实现，不依赖 libresolv 的 `ns_initparse`/`ns_parserr`。
