...
> **Linux 用户注意：** mDNS 解析需要客户端支持多播网络功能。请确保已安装并运行 `avahi-daemon`，或 `systemd-resolved` 已启用 mDNS 功能（在 `resolved.conf` 中设置 `MulticastDNS=yes`）。缺少其中之一可能导致 mDNS 查询失败。
