...
**HTTP 抽象层：** 所有服务商 API 通信均通过 `IHttpSender` 接口进行。具体实现 `HttpClient` 在内部封装了 [cpp-httplib](https://github.com/yhirose/cpp-httplib)，`httplib` 类型不会暴露在任何公开头文件中。核心根据每个子域名的配置绑定正确的网络接口和地址族，然后将 `IHttpSender` 注入到驱动调用中。
